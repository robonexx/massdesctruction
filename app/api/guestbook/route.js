...
import { getGuestbookCollection, MongoConfigurationError } from '../../lib/mongodb';
...
export async function GET() {
  try {
    ...
    const entries = await guestbook.find({}).sort({ _id: -1 }).toArray();
...
export async function PATCH(request) {
  try {
    ...
    const entry = await guestbook.findOne({ _id: new ObjectId(id) });
...
export async function DELETE(request) {
  try {
    ...
    if (!ObjectId.isValid(id)) return Response.json({ error: 'Invalid id' }, { status: 400 });
    const guestbook = await getGuestbookCollection();
    await guestbook.deleteOne({ _id: new ObjectId(id) });
